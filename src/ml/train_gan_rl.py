...
MODEL_DIR = f"../data/model/{CONFIG['version']}_rl"
...
print(f"Using dataset subset: {len(dataset)} images")
...
for epoch in range(CONFIG["num_epochs"]):
    ...
    for phase in ["train","val"]:
        ...
        for imgs, _ in tqdm(loader, desc=f"{phase} epoch {epoch+1}", leave=False):
            ...
            for _ in range(n_critic):
                ...
                gen_imgs = G(z)
                loss_D = (criterion(D(imgs), valid) + criterion(D(gen_imgs.detach()), fake)) / 2
            ...
            with torch.no_grad():
                fake_corrects.append((D(gen_imgs) < 0.5).float().mean().item())
        ...
        print(f"[{phase.upper()}] lossD={lossD:.3f} lossG={lossG:.3f} FID={FID:.1f} IS={IS:.2f} fake_corr={fake_corr:.3f} reward={reward:.3f}")
        ...
        if phase == "val":
            state = torch.tensor([FID/300, IS/5, fake_corr,
                      ...
                      n_critic/10, g_steps/5], device=device, dtype=torch.float)

            action, logp = agent.select_action(state.unsqueeze(0))
            optimizer_G, optimizer_D, n_critic, g_steps = apply_rl_action(
                action.squeeze().tolist(), optimizer_G, optimizer_D, n_critic, g_steps, min_lr, max_lr)
            agent.update(logp, reward)
            print(f"RL action applied: n_critic={n_critic}, g_steps={g_steps}, lr_G={optimizer_G.param_groups[0]['lr']:.6f}, lr_D={optimizer_D.param_groups[0]['lr']:.6f}")
    ...
    # --- Save models & sample images every 5 epochs ---
    if (epoch+1) % 5 == 0:
